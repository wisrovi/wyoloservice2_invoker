...
### Sequential Container Execution Process
During the lifecycle of a training task on a worker node, the Invoker orchestrates the execution using strictly sequential, ephemeral Docker containers spawned from the `executor` image. This guarantees environment isolation and memory recycling between phases:

1. **Container #1 (EDA Phase - `eda.py`)**: 
   First, an ephemeral container is launched to mount the shared network storage and execute the `DatasetAnalyzer`. It extracts dataset physical characteristics, distribution, bounding box metrics, and generates the initial Exploratory Data Analysis reports (`EDA_Report.docx`, `.md`, and plots). Upon completion, this container is destroyed.
2. **Container #2 (Training & LLM Phase - `run_training.py`)**: 
   Immediately after a successful EDA, a second container is spawned with strict GPU limits and permissions to run the heavy training (`trainer_wrapper.py`). It trains the model, saves weights, logs all results and EDA artifacts to MLflow, and executes the local OpenCode LLM integration to write the final `llm.md` insights report. Upon completion, this container is also destroyed.
3. **Passive Reading Phase (`llm_analizer.py`)**: 
   The final pipeline state does *not* create a third container. The Invoker natively reads the `llm.md` report from the shared volume generated by Container #2, injecting its content into the Celery task state for the UI and API to render.
...
### Version 1.9.3 (Current Release) - 2026-08-25
*   **Optuna study naming fixed:** `optuna_search` now reads the study name from `sweeper.study_name` (falling back to top-level `study_name`, then the dated default). The user-provided name was previously read from the wrong nesting level, so every run created a fresh `study_YYYYMMDD_<hash>` study and Optuna lost its cross-run trial history (warm-start).
*   **Version Update:** Bumped invoker version to v1.9.3.

### Version 1.9.2 - 2026-08-24
